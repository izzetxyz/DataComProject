...
constexpr int crcTableSize = 256;
uint32_t crcTable[crcTableSize];
...
void receiveChat(int socket)
{
  ...
  while (true)
  {
    ...
    if (command == "MESG")
    {
      ...
      uint32_t checksum = std::stoul(parts[1]);
      uint32_t actualChecksum = 0;
      for (int i = 2; i < parts.size(); i++)
      {
        for (const auto &c : parts[i])
        {
          actualChecksum = (actualChecksum >> 8) ^ crcTable[(actualChecksum ^ c) & 0xFF];
        }
      }
